        ...
        if line.starts_with("fn ") {
            ...
            while i < lines.len() {
                ...
                if body_line.starts_with("fn ") || 
                   (body_line.contains(":int =") || body_line.contains(":float =") || 
                    body_line.contains(":str =") || body_line.contains(":list =")) {
                    break;
                }
...
fn transpile_if_block(lines: &[&str], indent_level: usize) -> (String, usize) {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("if ") || line.starts_with("fn ") || 
           (line.contains(':') && line.contains('=') && 
            !line.starts_with("    ") && 
            (line.contains(":int") || line.contains(":float") || 
             line.contains(":str") || line.contains(":list"))) {
            break;
        }
...
fn transpile_type_cast(var_name: &str, var_type: &str, cast_call: &str) -> String {
    let value = extract_string(cast_call).unwrap_or("0");
    let rust_type = convert_type(var_type);
    
    match rust_type {
        "f64" | "f32" => {
            format!("let {}: {} = \"{}\".parse().unwrap_or(0.0);", var_name, rust_type, value)
        }
        "i32" | "i64" => {
            format!("let {}: {} = \"{}\".parse().unwrap_or(0);", var_name, rust_type, value)
        }
        _ => {
            format!("let {}: {} = \"{}\".to_string();", var_name, rust_type, value)
        }
    }
}
